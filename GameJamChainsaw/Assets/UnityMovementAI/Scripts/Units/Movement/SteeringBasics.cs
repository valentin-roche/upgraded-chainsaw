        ...
        public void Steer(Vector3 linearAcceleration)
        {
            if (windDurationLeft <= 0)
            {
                ...
                if (feetAnimator != null)
                    feetAnimator.SetFloat("magnitude", rb.Velocity.magnitude);
                else
                    print("Il faut set l'animator des pieds dans SteeringBasics");
            }
            else
            {
                windDurationLeft -= Time.deltaTime;
                //rb.Velocity = -rb.Velocity * pushFactor;

                Vector2 direction = playerTransform.position - transform.position;
                Vector2 movement = direction.normalized;
                movement = -direction * pushFactor;

                enemyBody.MovePosition((Vector2)transform.position + (movement * Time.deltaTime));
            }
